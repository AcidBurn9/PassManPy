...
class PassMan:
    def __init__(self, db_path: (str | None) = None):
        self.DB_PATH = Path(db_path) if db_path is not None else Path(__file__).parent / "vault.db"
    ...
    def init_db(self):
        logging.info(f"Initialising the database: {self.DB_PATH}")
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()

                cursor.execute(PassMan._DB_TABLE_USERS)
                cursor.execute(PassMan._DB_TABLE_PASSWORDS)

                db.commit()
        except Exception as e: logging.error(f"Failed to initialise the database: {self.DB_PATH}! ({e})")
    ...
    def _hash_pass(password: str) -> bytes:
        return PassMan._PASSWORD_HASHER.hash(password.encode())


    # Users
    def create_user(self, username: str, password: str) -> RegStatus:
        logging.debug(f"NEW_USER attempt for username={username}")
        salt = token_bytes(16)
        priv_bytes = PassMan._derive_key(password, salt)
        ...
        try:
            with self._get_db_connection() as db:
                ...
                cursor.execute(
                    ...
                    (username, PassMan._hash_pass(password), pub.public_bytes_raw(), salt),
                )
                db.commit()
    ...
    def auth_user(self, username: str, password: str) -> (int | None):
        ...
        try:
            PassMan._PASSWORD_HASHER.verify(passhash, password)
    ...
    def _get_user_pub(self, uid: int) -> (X25519PublicKey | None):
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()
    ...
    def _get_password_data(self, pid: int) -> (tuple[str, str, bytes, bytes, bytes, str, bytes] | None):
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()
                cursor.execute("""
    ...
    def get_password_owner(self, pid: int) -> (int | None):
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()
    ...
    def decrypt_password(self, pid: int, masterpass: str) -> (str | None):
        logging.debug(f"Password DECRYPT attempt for pid={pid}")
        password_data = self._get_password_data(pid)
        ...
        uid = self.auth_user(username, masterpass)
        if not uid:
        ...
        priv_bytes = PassMan._derive_key(masterpass, salt)
    ...
    def add_password(self, uid: int, label: str, login: str, password: str) -> bool:
        logging.debug(f"Password ADD attempt by uid={uid}")
        pub = self._get_user_pub(uid)
        if pub is None:
            logging.warning(f"Failed ADD by uid={uid} (no public key)")
            return False
        ct, nonce, eph_pub = self._encrypt_password(pub, password, label, login)
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()
                cursor.execute(
    ...
    def delete_password(self, pid: int, masterpass: str) -> bool:
    ...
    def update_password(self, pid: int, masterpass: str, new_password: str) -> bool:
        ...
        if pub is None:
            ...
            return False
        ct, nonce, eph_pub = PassMan._encrypt_password(pub, new_password, label, login)
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()
    ...
    """
    def search_passwords(self, uid: int, search_query: str = "") -> List[Tuple[int, str, str]]:
        ...
        try:
            with self._get_db_connection() as db:
                cursor = db.cursor()
                if search_query:
                    param = f"%{PassMan._escape_like(search_query)}%"
                    cursor.execute(PassMan.GET_PASSWORDS_QUERY_WITH_FILTER, (uid, param, param))
                else:
                    cursor.execute(PassMan.GET_PASSWORDS_QUERY, (uid,))
